...
const AUDIT_LOG_STORAGE_KEY = 'hack_pay_issuer_audit_logs'
if (typeof window !== 'undefined') {
  migrateLocalKey('prize_vault_issuer_audit_logs', AUDIT_LOG_STORAGE_KEY)
  migrateLocalKey('twin_lock_issuer_audit_logs', AUDIT_LOG_STORAGE_KEY)
}
...
export function appendIssuerAuditLog(entry) {
  ...
  try {
    window.dispatchEvent(new CustomEvent('hack_pay_audit_logs_updated'))
